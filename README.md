...
- [ ] :yellow_circle: **Multi-row insert:** If a COPY command is not an option and you require SQL inserts, use a multi-row insert whenever possible.

  - :book: [Use a multi-row insert](https://docs.aws.amazon.com/redshift/latest/dg/c_best-practices-multi-row-inserts.html)

- [ ] :yellow_circle: **Sort key order:** Load your data in sort key order to avoid needing to vacuum.
...
- [ ] :red_circle: **Automatic workload management (WLM):** Amazon Redshift determines how many concurrent queries and how much memory is allocated to each dispatched query.
...
- [ ] :yellow_circle: **Concurrency scaling:** Dynamically adds concurrent clusters improving read query concurrency.

  - :book: [Working with concurrency scaling](https://docs.aws.amazon.com/redshift/latest/dg/concurrency-scaling.html)

- [ ] :yellow_circle: **:new: AZ64 column compression encoding:** Consider using Redshift's proprietery new column encoding algorithm AZ64.
...
- [ ] :red_circle: **Enhanced VPC routing:** Forces all COPY and UNLOAD traffic between your cluster and your data repositories through your Amazon VPC.

  - :book: [Amazon Redshift enhanced VPC routing](https://docs.aws.amazon.com/redshift/latest/mgmt/enhanced-vpc-routing.html)

- [ ] :red_circle: **User groups:** Create different user groups and grant privileges based on their roles.
...
- [ ] :yellow_circle: **:new: Federated user access:** Consider providing user access via SAML-2.0 using AD FS, PingFederate, Okta, or Azure AD.
...
- [ ] :yellow_circle: **:new: Multi-factor authentication (MFA):** Consider enabling MFA for production workloads.
...
- [ ] :red_circle: **Long running queries:** Set an alarm to notify users when queries are running for longer than expected using the QueryDuration CloudWatch metric.
...
- [ ] :red_circle: **Disk space usage:** Check if your cluster is running out of disk space and whether you need to consider scaling using the PercentageDiskSpaceUsed metric.

  - :book: [Amazon Redshift performance data](https://docs.aws.amazon.com/redshift/latest/mgmt/metrics-listing.html)

- [ ] :red_circle: **:new: CloudWatch anomaly detection:** Applies machine-learning algorithms to the metric's past data to create a model of the metric's expected values.
...
- [ ] :yellow_circle: **Workload performance:** Optimise your cluster based on how much time queries spend on different stages of processing.

  - :book: [Analyzing workload performance](https://docs.aws.amazon.com/redshift/latest/mgmt/analyze-workload-performance.html)

- [ ] :yellow_circle: **Advanced Redshift monitoring:** Use Lambda and CloudWatch events to generate alarms for common possible issues.
...
- [ ] :red_circle: **Automated snapshot retention:** The default retention period of 1 day can catch organisations out in case of disaster recovery or rollback. Conside changing to 35 days.
...
- [ ] :yellow_circle: **:new: RA3 nodes** Consider using Redshift's new RA3 nodes with a mix of local cache and S3 backed elastic storage if compute requirements exceed dense compute or dense storage node levels.
